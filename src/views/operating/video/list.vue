...
<template>
    <div>
        <Card>
            <p slot="title"><Icon type="pinpoint"></Icon> 视频预约</p>
            <Row>
...
<script>
    ...
    export default {
        ...
        methods: {
            getData () {
                this.$ajax.get(userServicies(), {params: {page: 1, size: 50, type: 3}}).then((res) => {
            ...
            openService (qid, index) {
                const _this = this;
                this.changeStatus(1, function () { _this.data1[index].status = 1; _this.$Message.success({ content: '您开始了对' + _this.data1[index].name + '的服务，请尽快安排视频', duration: 3 }); }, qid);
            },
